...
/* BLES_WINE_PROCESS_RUNTIME_PREPARE_IMPL_20260723
 *
 * Esta función puede entrar a FAT/VFS y por eso debe ejecutarse antes de que
 * el loader retenga la preempción para publicar atómicamente un proceso.
 */
bool win32_process_prepare_runtime(void) {
    if (vfs_mkdir("/TEMP")) return true;

    kprintf("[WIN32] warning: TEMP no disponible\n");
    return false;
}
...
void win32_process_destroy(uint32_t pid) {
    /* BLES_WINE_PROCESS_DESTROY_IDEMPOTENT_20260723 */
    win32_process_environment_t *environment;
    void *teb = NULL;
    void *peb = NULL;
    void *ldr = NULL;
    ...
    kmemset(static_blocks, 0, sizeof(static_blocks));

    win32_user32_cleanup_process(pid);
    win32_gdi_cleanup_process(pid);
    win32_vm_cleanup_process(pid);
    win32_process_handle_cleanup_process(pid);
    win32_exception_cleanup_process(pid);
    win32_msvcrt_cleanup_process(pid);
    win32_winsock_cleanup_process(pid);
    win32_winmm_cleanup_process(pid);
    win32_ole32_cleanup_process(pid);
    win32_lz32_cleanup_process(pid);
    win32_wininet_cleanup_process(pid);
    win32_dx_cleanup_process(pid);
    win32_legacy_cleanup_process(pid);
    win32_kernel32_cleanup_process(pid);
    ...
    if (environment) {
        ...
        for (uint32_t i = 0; i < WIN32_STATIC_TLS_MODULES; i++) {
            static_blocks[i] = environment->static_tls[i].main_block;
            environment->static_tls[i].main_block = NULL;
        }
    ...
    for (uint32_t i = 0; i < WIN32_STATIC_TLS_MODULES; i++) {
        if (static_blocks[i] && mm_allocation_size(static_blocks[i]))
            kfree(static_blocks[i]);
    }

    if (teb && mm_allocation_size(teb)) kfree(teb);
    if (peb && mm_allocation_size(peb)) kfree(peb);
    if (ldr && mm_allocation_size(ldr)) kfree(ldr);
    if (parameters && mm_allocation_size(parameters)) kfree(parameters);
}
