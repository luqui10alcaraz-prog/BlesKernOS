...
static int WIN32_API win32_EnableWindow(void *hwnd, int enable);
...
static void cleanup_window(void *hwnd) {
    ...
    void *modal_owner = NULL;
    if (!w) return;
    if (w->dialog && w->dialog_modal && w->dialog_owner_was_enabled &&
        window_from_handle(w->parent))
        modal_owner = w->parent;
    ...
    if (w->window_extra) kfree(w->window_extra);
    kmemset(w, 0, sizeof(*w));
    if (modal_owner && window_from_handle(modal_owner)) {
        (void)win32_EnableWindow(modal_owner, 1);
        (void)win32_SetForegroundWindow(modal_owner);
    }
    gui_request_paint();
}
...
static int WIN32_API win32_ShowWindow(void *hwnd, int command) {
    ...
    } else if (command == SW_HIDE) {
        w->native->visible = false;
        w->native->listed = false;
        w->native->minimized = false;
        gui_desktop_window_hidden(gui_get_desktop(), w->native);
    } else if (command == SW_SHOWMINIMIZED || command == SW_MINIMIZE ||
               command == SW_SHOWMINNOACTIVE || command == SW_FORCEMINIMIZE) {
        gui_window_minimize(w->native);
    } else {
        gui_window_restore(w->native);
        /*
...
static void win32_pump_timers_and_destroy(void) {
    ...
    /* A native frame can be invisible because it is hidden or minimized.
       Only gui_window_close() also clears listed while Win32 still considers
       the HWND visible; that combination represents the desktop close button
       and must synthesize WM_DESTROY. */
    for (uint32_t i = 0; i < WIN32_MAX_WINDOWS; i++) {
        win_window_t *w = &win_windows[i];
        if (w->used && !w->control && w->native && w->visible &&
            !w->native->visible && !w->native->listed && !w->destroy_sent) {
            w->destroy_sent = true;
            queue_message((void *)(uintptr_t)(HWND_BASE + i),
                          WM_DESTROY, 0, 0);
        }
    }
}
...
static int WIN32_API win32_SetWindowPos(void *hwnd, void *after,
    ...
    if (flags & SWP_HIDEWINDOW) {
        ...
        if (!w->control && w->native) {
            ...
            w->native->listed = false;
            w->native->minimized = false;
            gui_desktop_window_hidden(desktop, w->native);
        }
    }
    ...
    if (!(flags & SWP_NOACTIVATE)) {
        if (!w->control && desktop && w->visible) {
...
static int WIN32_API win32_EnableWindow(void *hwnd, int enable) {
    ...
    if (!w) return 0;
    was_disabled = !w->enabled;
    w->enabled = enable != 0;
    if (!w->enabled) w->focused = false;
    if (!w->control && w->native) {
        w->native->input_enabled = w->enabled;
        if (!w->enabled)
            gui_desktop_window_hidden(gui_get_desktop(), w->native);
        w->native->dirty = true;
    }
...
static int WIN32_API win32_SetForegroundWindow(void*hwnd){win_window_t*w=window_from_handle(hwnd);gui_desktop_t*d=gui_get_desktop();if(!w||w->control||!w->enabled||!w->native||!d)return 0;gui_desktop_raise_window(d,w->native);gui_desktop_focus_window(d,w->native);gui_request_paint();return 1;}
...
static int WIN32_API win32_EndDialog(void *hwnd, int result) {
    ...
    if (dialog->native) gui_window_close(dialog->native);
    gui_request_paint();
    return 1;
}
...
int WIN32_API win32_user_dialog_modal_step(
    ...
    if (dialog->dialog_done) {
        ...
        /* cleanup_window() also restores and activates a disabled modal owner,
           including DestroyWindow paths that bypass EndDialog. */
        cleanup_window(hwnd);
