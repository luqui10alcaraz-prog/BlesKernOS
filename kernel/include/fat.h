...
/* Monta un dispositivo o particion FAT en una estructura independiente sin
 * reemplazar el volumen activo del sistema. */
...
/* Diagnostico del ultimo formato. FAT_FORMAT_LBA_NONE indica que el error
 * ocurrio antes de empezar a escribir sectores. */
#define FAT_FORMAT_LBA_NONE 0xFFFFFFFFU
const char *fat_format_last_error(void);
uint32_t fat_format_last_error_lba(void);
/* Invalida cualquier montaje/cache asociado a un dispositivo que Setup va a
 * destruir. No formatea ni escribe por si sola. */
...
/* Diagnostico de la ultima operacion FAT que fallo. */
...
/* Setup/BKL bulk-write accelerator. Keeps the volume FAT in RAM between
 * consecutive file creations so each extracted file does not reread the full
 * allocation table. Changes are still committed per file for crash safety. */
