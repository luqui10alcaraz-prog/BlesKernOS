...
/* Algunos 8042 compatibles (y ciertas combinaciones BIOS/hipervisor) aceptan
 * el modo streaming pero no vuelven a entregar IRQ12 despues de cambiar el
 * modo de video. Este respaldo solo consume 0x60 cuando el controlador marca
 * explicitamente que el byte pertenece al puerto auxiliar. */
static void mouse_poll_aux_bytes(void) {
    for (uint8_t count = 0; count < 12U; count++) {
        uint8_t status = inb(KBD_STATUS_PORT);
        if ((status & (PS2_STATUS_OUTPUT_FULL | PS2_STATUS_AUX_DATA)) !=
            (PS2_STATUS_OUTPUT_FULL | PS2_STATUS_AUX_DATA))
            break;
        mouse_consume_byte(inb(KBD_DATA_PORT));
    }
}
