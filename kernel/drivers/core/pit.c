...
static uint32_t pit_frequency_hz = 300U;
...
void pit_init(void) {
    ...
    pit_frequency_hz = compat_mode_timer_hz();
    if (pit_frequency_hz < 50U) pit_frequency_hz = 50U;
    if (pit_frequency_hz > 1000U) pit_frequency_hz = 1000U;
    divisor = (uint16_t)(1193182U / pit_frequency_hz);
