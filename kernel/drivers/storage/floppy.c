...
#define FDC_CMD_WRITE_DATA      0x45  /* WRITE DATA + MFM, DMA transfer */
...
static void dma_floppy_setup_write(uint32_t phys_addr, uint16_t size) {
    uint16_t count = (uint16_t)(size - 1);
    ...
    /* Single transfer, memoria -> dispositivo, canal 2. */
...
static bool floppy_write_one(uint32_t lba, const void *buffer) {
    uint8_t cyl;
    uint8_t head;
    uint8_t sector;
    uint8_t result[7] = {0};
    ...
    kmemcpy(FLOPPY_DMA_BUFFER, buffer, BLOCK_SECTOR_SIZE);
    dma_floppy_setup_write(FLOPPY_DMA_BUFFER_PHYS, BLOCK_SECTOR_SIZE);
    if (!fdc_write(FDC_CMD_WRITE_DATA)) goto fail;
    if (!fdc_write((uint8_t)((head << 2) | 0))) goto fail;
    if (!fdc_write(cyl)) goto fail;
    if (!fdc_write(head)) goto fail;
    if (!fdc_write(sector)) goto fail;
    if (!fdc_write(2)) goto fail;
    if (!fdc_write(FLOPPY_SECTORS_PER_TRACK)) goto fail;
    if (!fdc_write(0x1B)) goto fail;
    if (!fdc_write(0xFF)) goto fail;
    if (!fdc_read_result(result)) goto fail;
...
fail:
    kprintf("[FDC] write fail lba=%u chs=%u/%u/%u st=0x%x 0x%x 0x%x\n",
            lba, cyl, head, sector, result[0], result[1], result[2]);
    return false;
}

static bool floppy_write_block(block_device_t *dev UNUSED, uint32_t lba,
                               uint8_t count, const void *buffer) {
    const uint8_t *src = (const uint8_t *)buffer;
    if (!buffer || !count || lba + count > FLOPPY_SECTOR_COUNT) return false;

    floppy_motor_on();
    for (uint8_t i = 0; i < count; i++) {
        bool ok = false;
        for (uint8_t retry = 0; retry < 5 && !ok; retry++) {
            ok = floppy_write_one(lba + i,
                                  src + (uint32_t)i * BLOCK_SECTOR_SIZE);
            if (!ok) {
                fdc_reset();
                (void)floppy_recalibrate();
            }
...
void floppy_init(void) {
    ...
    pic_unmask_irq(6);
    block_register_ex("fd0", BLOCK_DEVICE_FLOPPY, FLOPPY_SECTOR_COUNT,
                      BLOCK_SECTOR_SIZE, false, NULL, floppy_read_block);
    (void)block_set_writer("fd0", floppy_write_block);
    kprintf("  [FDC] fd0: 1.44MB CHS 80/2/18 DMA lectura/escritura\n");
}
