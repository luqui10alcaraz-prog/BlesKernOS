...
void gfx_vga_blit_12h(const gfx_info_t *info, const uint32_t *pixels,
    ...
    if (!info || info->mode != GFX_MODE_VGA_12H || !pixels || !source_pitch ||
        w <= 0 || h <= 0) return;
    if (x < 0) { w += x; x = 0; }
    if (y < 0) { h += y; y = 0; }
    ...
    /* Write one complete byte for each group of eight pixels on each plane.
       The old per-pixel path reprogrammed the VGA GC for every pixel and made
       640x480x4 effectively unusable on a Pentium III. */
    vga_write_indexed(VGA_GC_INDEX, 0x05, 0x00);
    vga_write_indexed(VGA_GC_INDEX, 0x08, 0xFF);
    for (uint8_t plane = 0; plane < 4; plane++) {
        vga_write_indexed(VGA_SEQ_INDEX, 0x02, (uint8_t)(1U << plane));
        for (int row = y; row < y + h; row++) {
            volatile uint8_t *dst = (volatile uint8_t *)(VGA_GRAPHICS_FB +
                (uint32_t)row * VGA_12H_PITCH + (uint32_t)(x >> 3));
            int first_byte = x >> 3;
            int last_byte = (x + w - 1) >> 3;
            for (int byte = first_byte; byte <= last_byte; byte++) {
                ...
                for (int bit = 0; bit < 8; bit++) {
                    ...
                    uint8_t color = gfx_vga_rgb_to_color(
                        pixels[(uint32_t)row * source_pitch + (uint32_t)px]);
                    if (color & (uint8_t)(1U << plane))
                        value |= (uint8_t)(0x80U >> bit);
                }
                dst[byte - first_byte] = value;
            }
        }
    }
    vga_write_indexed(VGA_SEQ_INDEX, 0x02, 0x0F);
