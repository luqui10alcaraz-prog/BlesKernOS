...
typedef struct {
    ...
    uint32_t root_device_id[UHCI_ROOT_PORTS];
    volatile bool transfer_locked;
    bool ready;
} uhci_controller_t;
...
#define USB_MAX_CLASS_DRIVERS 8U
static usb_device_t g_usb_devices[BK_USB_MAX_DEVICES];
static uint32_t g_usb_next_id = 1U;
static const usb_class_driver_t *g_usb_class_drivers[USB_MAX_CLASS_DRIVERS];
static uint32_t g_usb_class_driver_count;
...
static bool uhci_transfer_lock(uhci_controller_t *hc) {
    if (!hc) return false;
    for (;;) {
        bool acquired = false;
        task_preempt_disable();
        if (!hc->transfer_locked) {
            hc->transfer_locked = true;
            acquired = true;
        }
        task_preempt_enable();
...
static void uhci_transfer_unlock(uhci_controller_t *hc) {
    if (!hc) return;
    task_preempt_disable();
    hc->transfer_locked = false;
    task_preempt_enable();
}
...
static bool uhci_run_chain(uhci_controller_t *hc, uhci_td_t *head,
    ...
    /* Remove a timed-out queue from the live schedule first, then wait two
       frames before recycling its TDs. This is essential for interrupt IN:
       a normal NAK leaves the TD ACTIVE, but must not stop/restart the whole
       UHCI controller on every empty keyboard or mouse poll. */
    if (timed_out && uhci_chain_active(head)) {
        detach_frame = uhci_inw((uint16_t)(hc->io_base + UHCI_FRNUM));
        uhci_park_schedule(hc);
        uhci_dma_fence();
        detached_for_abort = uhci_wait_frame_advance(hc, detach_frame, 2U);
        if (!detached_for_abort) {
    ...
    if (!detached_for_abort && !stopped_for_abort &&
...
static bool usb_request(uhci_controller_t *hc, uint8_t addr,
    ...
    setup.len = len;
    bool ok;
    if (!uhci_transfer_lock(hc)) return false;
    ok = uhci_control(hc, addr, max_packet, &setup, data, len,
                      low_speed, true);
    uhci_transfer_unlock(hc);
    return ok;
}
...
static bool uhci_bulk_unlocked(uhci_mass_device_t *dev, bool in, void *data,
...
static bool uhci_bulk(uhci_mass_device_t *dev, bool in, void *data,
                      ...
                      uint32_t timeout_ms, uint32_t *actual_len) {
    bool ok;
    if (!dev || !dev->hc || !uhci_transfer_lock(dev->hc)) return false;
    ok = uhci_bulk_unlocked(dev, in, data, len, stage, timeout_ms,
                            actual_len);
    uhci_transfer_unlock(dev->hc);
    return ok;
}
...
static usb_device_t *usb_core_find_id(uint32_t id) {
    if (!id) return NULL;
    for (uint32_t i = 0; i < BK_USB_MAX_DEVICES; i++)
        if (g_usb_devices[i].connected && g_usb_devices[i].id == id)
            return &g_usb_devices[i];
    return NULL;
}

static usb_device_t *usb_core_alloc_device(void) {
    for (uint32_t i = 0; i < BK_USB_MAX_DEVICES; i++) {
        if (!g_usb_devices[i].connected) {
            kmemset(&g_usb_devices[i], 0, sizeof(g_usb_devices[i]));
            g_usb_devices[i].core_abi = BK_USB_CORE_ABI_VERSION;
            g_usb_devices[i].id = g_usb_next_id++;
            if (!g_usb_next_id) g_usb_next_id = 1U;
            return &g_usb_devices[i];
        }
    }
    return NULL;
...
static void usb_core_probe_interface(usb_device_t *device,
                                     usb_interface_t *interface) {
    if (!device || !interface || interface->claimed ||
        interface->alternate != 0U) return;
    for (uint32_t i = 0; i < g_usb_class_driver_count; i++) {
        const usb_class_driver_t *driver = g_usb_class_drivers[i];
        if (driver && driver->probe && driver->probe(device, interface)) {
            interface->claimed = true;
            interface->owner = driver;
            kprintf("  [USB] %s reclamo dev=%u intf=%u class=%x/%x/%x\n",
                    driver->name ? driver->name : "class-driver",
                    device->id, interface->number, interface->class_code,
...
static void usb_core_probe_device(usb_device_t *device) {
    if (!device || !device->connected) return;
    for (uint32_t i = 0; i < device->interface_count; i++)
        usb_core_probe_interface(device, &device->interfaces[i]);
}

static void usb_core_disconnect(usb_device_t *device) {
    uint32_t id;
    if (!device || !device->connected) return;
    id = device->id;
    for (uint32_t i = 0; i < BK_USB_MAX_DEVICES; i++) {
        if (g_usb_devices[i].connected && g_usb_devices[i].parent_id == id)
            usb_core_disconnect(&g_usb_devices[i]);
    }
    for (uint32_t i = 0; i < device->interface_count; i++) {
        usb_interface_t *interface = &device->interfaces[i];
        const usb_class_driver_t *driver =
            (const usb_class_driver_t *)interface->owner;
        if (interface->claimed && driver && driver->disconnect)
            driver->disconnect(device, interface);
        interface->claimed = false;
        interface->owner = NULL;
    }
    kprintf("  [USB] desconectado dev=%u addr=%u vid=%x pid=%x\n",
            device->id, device->address, device->vendor_id,
            device->product_id);
    device->connected = false;
}

bool usb_class_register(const usb_class_driver_t *driver) {
    if (!driver || driver->abi_version != BK_USB_CORE_ABI_VERSION ||
        driver->descriptor_size < sizeof(usb_class_driver_t) ||
        !driver->probe ||
        g_usb_class_driver_count >= USB_MAX_CLASS_DRIVERS) return false;
    for (uint32_t i = 0; i < g_usb_class_driver_count; i++)
        if (g_usb_class_drivers[i] == driver) return true;
    g_usb_class_drivers[g_usb_class_driver_count++] = driver;
    for (uint32_t i = 0; i < BK_USB_MAX_DEVICES; i++)
        if (g_usb_devices[i].connected) usb_core_probe_device(&g_usb_devices[i]);
    return true;
}

bool usb_class_unregister(const usb_class_driver_t *driver) {
    uint32_t position = g_usb_class_driver_count;
    if (!driver) return false;
    for (uint32_t i = 0; i < g_usb_class_driver_count; i++) {
        if (g_usb_class_drivers[i] == driver) {
            position = i;
            break;
        }
    }
    if (position == g_usb_class_driver_count) return false;
    for (uint32_t d = 0; d < BK_USB_MAX_DEVICES; d++) {
        usb_device_t *device = &g_usb_devices[d];
        if (!device->connected) continue;
        for (uint32_t i = 0; i < device->interface_count; i++) {
            usb_interface_t *interface = &device->interfaces[i];
            if (!interface->claimed || interface->owner != driver) continue;
            if (driver->disconnect) driver->disconnect(device, interface);
            interface->claimed = false;
            interface->owner = NULL;
        }
    }
    for (uint32_t i = position + 1U; i < g_usb_class_driver_count; i++)
        g_usb_class_drivers[i - 1U] = g_usb_class_drivers[i];
    g_usb_class_drivers[--g_usb_class_driver_count] = NULL;
    return true;
}
...
uint32_t usb_device_count(void) {
    uint32_t count = 0;
    for (uint32_t i = 0; i < BK_USB_MAX_DEVICES; i++)
        if (g_usb_devices[i].connected) count++;
    return count;
}

usb_device_t *usb_device_at(uint32_t index) {
    for (uint32_t i = 0; i < BK_USB_MAX_DEVICES; i++) {
        if (!g_usb_devices[i].connected) continue;
        if (!index--) return &g_usb_devices[i];
    }
    return NULL;
}
...
usb_endpoint_t *usb_interface_endpoint(usb_interface_t *interface,
    ...
    for (uint32_t i = 0; i < interface->endpoint_count; i++) {
        ...
        if ((endpoint->attributes & 3U) != (type & 3U)) continue;
        if (USB_ENDPOINT_IN(endpoint->address) != in) continue;
        if (!ordinal--) return endpoint;
    }
    return NULL;
}
...
bool usb_control_transfer(usb_device_t *device, uint8_t request_type,
    ...
    uhci_controller_t *hc;
    if (!device || !device->connected || !device->hcd_private) return false;
    hc = (uhci_controller_t *)device->hcd_private;
    return usb_request(hc, device->address, device->max_packet0,
                       device->speed == USB_SPEED_LOW, request_type, request,
                       value, index, length, data);
}

static bool uhci_endpoint_batch(usb_device_t *device,
                                usb_endpoint_t *endpoint,
                                uint8_t *data, uint32_t length,
                                uint32_t *actual_length,
                                uint32_t timeout_ms) {
    uhci_controller_t *hc = (uhci_controller_t *)device->hcd_private;
    uint8_t pid = USB_ENDPOINT_IN(endpoint->address) ? UHCI_PID_IN
                                                     : UHCI_PID_OUT;
    uint8_t ep = USB_ENDPOINT_NUMBER(endpoint->address);
    uint8_t initial_toggle = endpoint->toggle;
    uint8_t toggle = initial_toggle;
    uint16_t max_packet = endpoint->max_packet;
    uhci_td_t *head = NULL;
    uhci_td_t *previous = NULL;
    uint32_t remaining = length;
    uint32_t packets = 0;
    uhci_chain_result_t result;
    bool interrupt = (endpoint->attributes & 3U) == USB_EP_INTERRUPT;

    if (actual_length) *actual_length = 0;
    if (!hc || !data || !length || !max_packet || max_packet > 64U)
        return false;
    while (remaining && packets < UHCI_MAX_TD) {
        ...
        if (!head) head = td;
        uhci_td_init(td, previous, pid, device->address, ep, toggle,
                     data, chunk, device->speed == USB_SPEED_LOW);
        if (pid == UHCI_PID_IN && remaining > chunk)
            td->status |= UHCI_TD_STS_SPD;
        previous = td;
        toggle ^= 1U;
        data += chunk;
    ...
    if (!uhci_run_chain(hc, head, !interrupt,
                        interrupt ? "INTERRUPT" :
                        (pid == UHCI_PID_IN ? "BULK-IN" : "BULK-OUT"),
                        timeout_ms ? timeout_ms :
                        (interrupt ? 2U : UHCI_BULK_TIMEOUT_MS), &result)) {
        if (interrupt && result.failed_valid &&
            (result.failed_status & UHCI_TD_STS_ACTIVE) &&
            !(result.failed_status & UHCI_TD_STS_FATAL)) {
            if (actual_length) *actual_length = 0;
            return true; /* NAK/no periodic report is not an I/O error. */
        }
        if (actual_length) *actual_length = result.actual_bytes;
        if (result.failed_valid) endpoint->toggle = result.failed_toggle;
        return false;
    }
    if (!result.completed_tds || result.completed_tds > packets) return false;
    endpoint->toggle = initial_toggle ^
                       (uint8_t)(result.completed_tds & 1U);
    if (actual_length) *actual_length = result.actual_bytes;
    return true;
}
...
bool usb_endpoint_transfer(usb_device_t *device, usb_endpoint_t *endpoint,
    ...
    bool in;
    if (actual_length) *actual_length = 0;
    if (!device || !device->connected || !endpoint || !data || !length ||
        !device->hcd_private) return false;
    if ((endpoint->attributes & 3U) != USB_EP_BULK &&
        (endpoint->attributes & 3U) != USB_EP_INTERRUPT) return false;
    hc = (uhci_controller_t *)device->hcd_private;
    in = USB_ENDPOINT_IN(endpoint->address);
    if (!uhci_transfer_lock(hc)) return false;
    while (length) {
        uint32_t batch = length;
        uint32_t actual = 0;
        if (batch > UHCI_CTRL_DMA_SIZE) batch = UHCI_CTRL_DMA_SIZE;
        if (in) kmemset(hc->control_buffer, 0, batch);
        else kmemcpy(hc->control_buffer, cursor, batch);
        uhci_dma_fence();
        if (!uhci_endpoint_batch(device, endpoint, hc->control_buffer,
                                 batch, &actual, timeout_ms)) {
            uhci_transfer_unlock(hc);
            if (actual_length) *actual_length = total + actual;
            return false;
        }
        uhci_dma_fence();
        if (actual > batch) {
        ...
        if (in && actual) kmemcpy(cursor, hc->control_buffer, actual);
        total += actual;
        if (actual_length) *actual_length = total;
        if (in && actual < batch) break;
        if (!in && actual != batch) {
            uhci_transfer_unlock(hc);
            return false;
        }
        cursor += in ? actual : batch;
        length -= batch;
    }
    uhci_transfer_unlock(hc);
    return true;
...
bool usb_clear_endpoint_halt(usb_device_t *device,
    ...
    if (!usb_control_transfer(device,
            RT_HOST_TO_DEV | RT_STANDARD | RT_ENDPOINT,
            REQ_CLEAR_FEATURE, USB_FEATURE_ENDPOINT_HALT,
            endpoint->address, NULL, 0)) return false;
    endpoint->toggle = 0;
    return true;
}
...
bool usb_set_interface(usb_device_t *device, uint8_t interface_number,
    ...
    return usb_control_transfer(device,
        ...
        REQ_SET_INTERFACE, alternate_setting, interface_number, NULL, 0);
}

static int32_t usb_core_hc_index(uhci_controller_t *hc) {
    for (uint32_t i = 0; i < UHCI_MAX_CONTROLLERS; i++)
        if (&g_uhci[i] == hc) return (int32_t)i;
    return -1;
}

static bool usb_core_attach_mass(usb_device_t *device,
                                 usb_interface_t *interface) {
    int32_t hc_index;
    uhci_mass_device_t *mass;
    usb_endpoint_t *bulk_in;
    usb_endpoint_t *bulk_out;
    if (!device || !interface || interface->class_code != USB_CLASS_MASS_STORAGE ||
        interface->subclass != USB_SUBCLASS_SCSI ||
        interface->protocol != USB_PROTOCOL_BULK_ONLY) return false;
    hc_index = usb_core_hc_index((uhci_controller_t *)device->hcd_private);
    if (hc_index < 0) return false;
    mass = &g_mass[hc_index];
    if (mass->registered) return false;
    bulk_in = usb_interface_endpoint(interface, USB_EP_BULK, true, 0);
    bulk_out = usb_interface_endpoint(interface, USB_EP_BULK, false, 0);
    if (!bulk_in || !bulk_out) return false;
    kmemset(mass, 0, sizeof(*mass));
    mass->hc = (uhci_controller_t *)device->hcd_private;
    mass->addr = device->address;
    mass->max_packet0 = device->max_packet0;
    mass->bulk_in = bulk_in->address;
    mass->bulk_out = bulk_out->address;
    mass->interface_number = interface->number;
    mass->interface_alternate = interface->alternate;
    mass->bulk_in_packet = bulk_in->max_packet;
    mass->bulk_out_packet = bulk_out->max_packet;
    if (!usb_msc_get_max_lun(mass) || !usb_msc_reset_recovery(mass) ||
        !usb_msc_read_capacity(mass) || !usb_choose_name(mass->name)) {
        kmemset(mass, 0, sizeof(*mass));
        return false;
    }
    if (!block_register_ex(mass->name, BLOCK_DEVICE_USB, mass->sectors,
                           BLOCK_SECTOR_SIZE, false, mass,
                           usb_block_read)) {
        kmemset(mass, 0, sizeof(*mass));
        return false;
    }
    block_set_writer(mass->name, usb_block_write);
    mass->registered = true;
    interface->claimed = true;
    kprintf("  [USB] %s: UHCI mass storage %u sectores\n",
            mass->name, mass->sectors);
    return true;
}

static void usb_core_parse_config(usb_device_t *device,
                                  const uint8_t *config,
                                  uint16_t total_length) {
    const uint8_t *cursor;
    const uint8_t *end;
    usb_interface_t *current = NULL;
    if (!device || !config || total_length < sizeof(usb_config_desc_t)) return;
    cursor = config + config[0];
    end = config + total_length;
    while (cursor + 2U <= end && cursor[0] >= 2U &&
           cursor + cursor[0] <= end) {
        if (cursor[1] == DESC_INTERFACE && cursor[0] >= 9U) {
            if (device->interface_count < BK_USB_MAX_INTERFACES) {
                current = &device->interfaces[device->interface_count++];
                kmemset(current, 0, sizeof(*current));
                current->number = cursor[2];
                current->alternate = cursor[3];
                current->class_code = cursor[5];
                current->subclass = cursor[6];
                current->protocol = cursor[7];
            } else current = NULL;
        } else if (current && cursor[1] == DESC_ENDPOINT && cursor[0] >= 7U) {
            if (current->endpoint_count < BK_USB_MAX_ENDPOINTS) {
                usb_endpoint_t *endpoint =
                    &current->endpoints[current->endpoint_count++];
                endpoint->address = cursor[2];
                endpoint->attributes = cursor[3];
                endpoint->max_packet = rd16(cursor + 4U) & 0x07FFU;
                endpoint->interval = cursor[6];
                endpoint->toggle = 0;
            }
        } else if (current && current->extra_length + cursor[0] <=
                              BK_USB_MAX_EXTRA) {
            kmemcpy(current->extra + current->extra_length,
                    cursor, cursor[0]);
            current->extra_length += cursor[0];
        }
        cursor += cursor[0];
    }
}

static uint8_t usb_core_allocate_address(uhci_controller_t *hc) {
    uint8_t candidate;
    if (!hc) return 0;
    if (!hc->next_address || hc->next_address > 127U) hc->next_address = 1U;
    candidate = hc->next_address;
    for (uint32_t attempt = 0; attempt < 127U; attempt++) {
        bool used = false;
        for (uint32_t i = 0; i < BK_USB_MAX_DEVICES; i++) {
            if (g_usb_devices[i].connected &&
                g_usb_devices[i].hcd_private == hc &&
                g_usb_devices[i].address == candidate) {
                used = true;
                break;
            }
        }
        hc->next_address = candidate == 127U ? 1U : (uint8_t)(candidate + 1U);
...
static usb_device_t *usb_core_enumerate(uhci_controller_t *hc,
                                        uint32_t parent_id,
                                        uint8_t parent_port,
                                        uint8_t root_port,
                                        uint8_t depth,
    ...
    uint8_t config[512];
    usb_config_desc_t *config_descriptor = (usb_config_desc_t *)config;
    usb_device_t *device;
    uint8_t address;
    uint16_t total_length;
    bool low_speed = speed == USB_SPEED_LOW;
    if (!hc || !hc->ready) return NULL;
    device = usb_core_alloc_device();
    if (!device) return NULL;
    kmemset(&descriptor, 0, sizeof(descriptor));
    if (!usb_request(hc, 0, 8, low_speed,
                     RT_DEV_TO_HOST | RT_STANDARD | RT_DEVICE,
                     REQ_GET_DESC, DESC_DEVICE << 8, 0, 8, &descriptor))
        goto failed;
    if (descriptor.max_packet0 != 8U && descriptor.max_packet0 != 16U &&
        descriptor.max_packet0 != 32U && descriptor.max_packet0 != 64U)
        descriptor.max_packet0 = 8U;
    address = usb_core_allocate_address(hc);
    if (!address) goto failed;
    if (!usb_request(hc, 0, descriptor.max_packet0, low_speed,
                     RT_HOST_TO_DEV | RT_STANDARD | RT_DEVICE,
                     REQ_SET_ADDR, address, 0, 0, NULL)) goto failed;
    usb_delay_ms(5);
    if (!usb_request(hc, address, descriptor.max_packet0, low_speed,
                     RT_DEV_TO_HOST | RT_STANDARD | RT_DEVICE,
                     REQ_GET_DESC, DESC_DEVICE << 8, 0,
                     sizeof(descriptor), &descriptor)) goto failed;
    kmemset(config, 0, sizeof(config));
    if (!usb_request(hc, address, descriptor.max_packet0, low_speed,
                     RT_DEV_TO_HOST | RT_STANDARD | RT_DEVICE,
                     REQ_GET_DESC, DESC_CONFIG << 8, 0,
                     sizeof(usb_config_desc_t), config)) goto failed;
    total_length = rd16(config + 2U);
    if (total_length < sizeof(usb_config_desc_t) ||
        total_length > sizeof(config)) goto failed;
    if (!usb_request(hc, address, descriptor.max_packet0, low_speed,
                     RT_DEV_TO_HOST | RT_STANDARD | RT_DEVICE,
                     REQ_GET_DESC, DESC_CONFIG << 8, 0,
                     total_length, config)) goto failed;
    if (!config_descriptor->config_value) goto failed;
    if (!usb_request(hc, address, descriptor.max_packet0, low_speed,
                     RT_HOST_TO_DEV | RT_STANDARD | RT_DEVICE,
                     REQ_SET_CONF, config_descriptor->config_value,
                     0, 0, NULL)) goto failed;
    usb_delay_ms(20);

    device->address = address;
    device->max_packet0 = descriptor.max_packet0;
    device->speed = speed;
    device->vendor_id = descriptor.vendor;
    device->product_id = descriptor.product;
    device->device_version = descriptor.device;
    device->device_class = descriptor.dev_class;
    device->device_subclass = descriptor.dev_subclass;
    device->device_protocol = descriptor.dev_protocol;
    device->configuration = config_descriptor->config_value;
    device->parent_id = parent_id;
    device->parent_port = parent_port;
    device->root_port = root_port;
    device->depth = depth;
    device->hcd_private = hc;
    usb_core_parse_config(device, config, total_length);
    device->connected = true;
    kprintf("  [USB] dev=%u addr=%u %s vid=%x pid=%x intf=%u depth=%u\n",
            device->id, address,
            low_speed ? "low-speed" : "full-speed",
            device->vendor_id, device->product_id,
            device->interface_count, depth);
    for (uint32_t i = 0; i < device->interface_count; i++)
        if (usb_core_attach_mass(device, &device->interfaces[i])) break;
    usb_core_probe_device(device);
    return device;
failed:
...
bool usb_enumerate_hub_port(usb_device_t *hub, uint8_t port,
                            usb_speed_t speed) {
    usb_device_t *child;
    if (!hub || !hub->connected || !hub->hcd_private || !port ||
        hub->depth >= 5U ||
        (speed != USB_SPEED_LOW && speed != USB_SPEED_FULL)) return false;
    usb_disconnect_hub_port(hub, port);
    child = usb_core_enumerate((uhci_controller_t *)hub->hcd_private,
                               hub->id, port, hub->root_port,
                               (uint8_t)(hub->depth + 1U), speed);
    return child != NULL;
}
...
void usb_disconnect_hub_port(usb_device_t *hub, uint8_t port) {
    ...
    for (uint32_t i = 0; i < BK_USB_MAX_DEVICES; i++) {
        if (g_usb_devices[i].connected &&
            g_usb_devices[i].parent_id == hub->id &&
            g_usb_devices[i].parent_port == port) {
            usb_core_disconnect(&g_usb_devices[i]);
            return;
        }
    }
...
static void __attribute__((unused)) usb_probe_low_speed_ep0(uhci_controller_t *hc, uint32_t port) {
    uint8_t desc[8];
    usb_setup_t setup;
    uint8_t bit;
...
static bool __attribute__((unused)) usb_try_mass_storage(uhci_controller_t *hc,
...
static void uhci_init_controller(const pci_device_t *pci) {
    ...
    for (uint32_t port = 0; port < hc->port_count; port++) {
        uint16_t portsc;
        usb_device_t *device;
        if (!uhci_reset_port(hc, port)) continue;
        uhci_log_port_state(hc, "after-reset");
        portsc = uhci_inw(uhci_port_addr(hc, port));
        device = usb_core_enumerate(hc, 0, 0, (uint8_t)(port + 1U), 0,
                    (portsc & UHCI_PORT_LOW_SPEED) ? USB_SPEED_LOW
                                                   : USB_SPEED_FULL);
        if (device) hc->root_device_id[port] = device->id;
    }
}
...
static void usb_uhci_scan_ports(void) {
    for (uint32_t index = 0; index < g_uhci_count; index++) {
        ...
        if (!hc->ready) continue;
        for (uint32_t port = 0; port < hc->port_count; port++) {
            uint16_t portsc = uhci_inw(uhci_port_addr(hc, port));
            usb_device_t *current = usb_core_find_id(hc->root_device_id[port]);
            if (!(portsc & UHCI_PORT_CONNECT)) {
                if (current) usb_core_disconnect(current);
                hc->root_device_id[port] = 0;
                hc->low_speed_probe_mask &= (uint8_t)~(1U << port);
                hc->swdbg_done_mask &= (uint8_t)~(1U << port);
                continue;
            }
            if (current) continue;
            if (uhci_reset_port(hc, port)) {
                ...
                portsc = uhci_inw(uhci_port_addr(hc, port));
                device = usb_core_enumerate(hc, 0, 0,
                            (uint8_t)(port + 1U), 0,
                            (portsc & UHCI_PORT_LOW_SPEED) ? USB_SPEED_LOW
                                                           : USB_SPEED_FULL);
                if (device) hc->root_device_id[port] = device->id;
...
void usb_uhci_init(void) {
    ...
    kmemset(g_usb_devices, 0, sizeof(g_usb_devices));
    kmemset(g_usb_class_drivers, 0, sizeof(g_usb_class_drivers));
    g_usb_class_driver_count = 0;
    g_usb_next_id = 1U;
