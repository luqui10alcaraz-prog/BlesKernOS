...
bool gui_change_resolution(gui_desktop_t *desktop, uint16_t width,
    ...
    use_16_colors = bpp == 4;
    hardware_bpp = use_16_colors ? 8 : bpp;
    ...
    /* The normal GUI loop performs the first complete frame after this API
       returns to user mode. That frame can safely deliver content callbacks. */
    gui_request_paint();
