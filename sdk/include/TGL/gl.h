...
/* BlesKernOS extension: fixed-function GFX3D status/control. TinyGL keeps
 * the OpenGL frontend and can fall back to its software rasterizer. */
...
/* Ends and submits the GPU frame without a GPU->CPU readback. */
GLint tglBlesKernOSGPUFinish(void);
/* BlesKernOS renderer policy and statistics. */
enum {
    TGL_BK_RENDERER_AUTO = 0,
    TGL_BK_RENDERER_CPU = 1,
    TGL_BK_RENDERER_GPU = 2
};

typedef struct tgl_bleskernos_gpu_stats {
    GLuint frames_started;
    GLuint frames_resolved;
    GLuint triangles_submitted;
    GLuint lines_submitted;
    GLuint points_submitted;
    GLuint draw_calls;
    GLuint texture_uploads;
    GLuint fallbacks;
} tgl_bleskernos_gpu_stats_t;
