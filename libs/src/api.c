...
#include "error_check_no_context.h"
    ...
    p[3].f = b;
    p[4].f = a;
    p[5].ui = (((GLuint) (r * COLOR_CORRECTED_MULT_MASK) + COLOR_MIN_MULT) &
               COLOR_MASK);
    p[6].ui = (((GLuint) (g * COLOR_CORRECTED_MULT_MASK) + COLOR_MIN_MULT) &
               COLOR_MASK);
    p[7].ui = (((GLuint) (b * COLOR_CORRECTED_MULT_MASK) + COLOR_MIN_MULT) &
               COLOR_MASK);
    gl_add_op(p);
}
...
#include "error_check_no_context.h"
    ...
    p[3].f = v[2];
    p[4].f = v[3];
    p[5].ui = (((GLuint) (v[0] * COLOR_CORRECTED_MULT_MASK) + COLOR_MIN_MULT) &
               COLOR_MASK);
    p[6].ui = (((GLuint) (v[1] * COLOR_CORRECTED_MULT_MASK) + COLOR_MIN_MULT) &
               COLOR_MASK);
    p[7].ui = (((GLuint) (v[2] * COLOR_CORRECTED_MULT_MASK) + COLOR_MIN_MULT) &
               COLOR_MASK);
...
#include "error_check_no_context.h"
    ...
    if (mode != GL_POINTS && mode != GL_LINES && mode != GL_LINE_LOOP &&
        ...
        mode != GL_TRIANGLE_STRIP && mode != GL_QUADS && mode != GL_QUAD_STRIP)
#define ERROR_FLAG GL_INVALID_ENUM
#include "error_check.h"
#endif
        gl_add_op(p);
...
#include "error_check.h"
    ...
    p[1].i = mode;
    p[2].i = type;
    n = 4; /* This appears to be a hack... to avoid a jump instruction? What the
              hell?*/
...
/* textures */
void glTexImage2D(GLint target,
                  GLint level,
                  GLint components,
                  GLint width,
                  GLint height,
...
void glTexImage1D(GLint target,
                  GLint level,
                  GLint components,
                  GLint width,
                  GLint border,
