...
/*	if(!(light >= GL_LIGHT0 && light < GL_LIGHT0 + MAX_LIGHTS)) return;*/
    ...
    for (i = 0; i < 4; i++)
        if (type != GL_POSITION && type != GL_SPOT_DIRECTION &&
            type != GL_SPOT_EXPONENT && type != GL_SPOT_CUTOFF &&
            type != GL_LINEAR_ATTENUATION && type != GL_CONSTANT_ATTENUATION &&
            type != GL_QUADRATIC_ATTENUATION)
