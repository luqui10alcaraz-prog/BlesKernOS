...
static void render_status(const char *status) {
    const gfx_info_t *info = gfx_get_info();
    int y0;
    int h;
    ...
    if (y0 < 0) y0 = 0;
    h = g_text_y + 18 - y0;
    if (y0 + h > (int)info->height) h = (int)info->height - y0;

    gfx_fill_rect_rgb(0, y0, (int)info->width, h, 0x00000000);
    draw_bar_blocks();
    if (status) draw_text_centered(g_text_y, status);
}

static void fade_up(const char *status) {
    render_full(status, 255);
}
...
void bootsplash_show(const char *status, uint8_t progress) {
    ...
    render_status(status ? status : "LOADING");
}
