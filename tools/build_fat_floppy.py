...
def bk_iconpak_try_prepare(argv):
    ...
    try:
        ...
        for i, arg in enumerate(list(argv)):
            ...
            bvi_tool = root / "tools" / "build_bvi_pak.py"
            bvi_dir = root / "assets" / "icons-bvi"

            if pak_only and bvi_dir.is_dir():
                pass
            elif tool.exists():
            ...
            if pak_only:
                # ATA/USB: empaqueta los documentos BVI1 crudos. No se
                # rasterizan: el sistema los escala al abrir cada icono.
                tmp = root / "build" / "icons_runtime"
                tmp.mkdir(parents=True, exist_ok=True)
                for old in tmp.iterdir():
                    if old.is_file():
                        old.unlink()
                if not bvi_tool.exists() or not bvi_dir.is_dir():
                    raise FileNotFoundError("faltan build_bvi_pak.py o assets/icons-bvi")
                subprocess.check_call([
                    sys.executable, str(bvi_tool), str(bvi_dir),
                    str(tmp / "ICONS.PAK")
                ])
                monitor_bmp = p / "MONITOR.BMP"
                if not monitor_bmp.is_file():
                    raise FileNotFoundError("falta assets/icons/MONITOR.BMP")
                shutil.copy2(monitor_bmp, tmp / "MONITOR.BMP")
                argv[i] = str(tmp)
...
def main():
    ...
    if len(sys.argv) < 2:
        print(
            ...
            "[system.cpl] [devmgr.cpl] [internet.cpl] [modem.cpl] [language.cpl] [ss_balls.o]",
            file=sys.stderr,
        )
        return 1
    ...
    ss_balls_path = sys.argv[32] if len(sys.argv) > 32 else None
    DATA_START_SECTOR = RESERVED_SECTORS + FAT_COUNT * FAT_SECTORS + ROOT_DIR_SECTORS
    ...
    if len(control_data) > 11:
        control_data[11] = omit_floppy_payload("LANGUAGE.CPL", control_data[11])
    ...
    icon_files = []
    icon_lookup = {}
    if icons_path and os.path.isdir(icons_path):
        icon_lookup = {name.upper(): name for name in os.listdir(icons_path)}
    for icon_name in (
        ...
        "INTERNET.BMP", "MODEM.BMP", "NETSCAPE.BMP", "MONITOR.BMP",
    ):
        actual_name = icon_lookup.get(icon_name.upper(), icon_name)
        icon_path = os.path.join(icons_path, actual_name) if icons_path else ""
