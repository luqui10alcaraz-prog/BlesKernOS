...
static void apitest_paint(gui_window_t *window UNUSED,
    ...
    bk_gui_surface_draw_text(surface, x, y, "@H9D38A563",
                             0x00002060, 0, false);
    row = y + 18;

    snprintf(line, sizeof(line), "@HD0915C64",
             bk_sys_api_version(), st->pid, bk_sys_uptime_ms());
    bk_gui_surface_draw_text(surface, x, row, line, 0x00202020, 0, false);
    row += 14;

    snprintf(line, sizeof(line), "@H39DC5FB9",
             st->total_kb, st->free_kb);
    bk_gui_surface_draw_text(surface, x, row, line, 0x00202020, 0, false);
    row += 14;

    snprintf(line, sizeof(line), "@H50226A38",
    ...
    snprintf(line, sizeof(line), "@H1972C9DA",
             st->screen_w, st->screen_h);
    bk_gui_surface_draw_text(surface, x, row, line, 0x00202020, 0, false);
    row += 14;

    snprintf(line, sizeof(line), "@HC643FDD8",
             st->task_count);
    bk_gui_surface_draw_text(surface, x, row, line, 0x00202020, 0, false);
    row += 14;

    if (st->rtc_ok) {
        snprintf(line, sizeof(line), "@H5D7BDC0A",
                 st->now.date.year, st->now.date.month, st->now.date.day,
                 st->now.time.hour, st->now.time.minute, st->now.time.second);
    } else {
        snprintf(line, sizeof(line), "@H81F1D8BE");
    }
    bk_gui_surface_draw_text(surface, x, row, line, 0x00202020, 0, false);
    row += 18;
...
static bool apitest_event(gui_window_t *window UNUSED,
    ...
    if (event->type == GUI_EVENT_KEY && (event->key == 'e' || event->key == 'E')) {
        st->alert_tested = bk_gui_alert(
            BK_ALERT_WARNING, "Prueba de avisos",
            "Este mensaje fue solicitado por una aplicacion mediante la API publica.",
            30);
        return true;
    }
    return false;
...
void bleskernos_program_main(gui_desktop_t *desktop) {
    ...
    st->window = bk_gui_create_window(desktop, 92, 76, 460, 210, "@H1A286185");
    if (!st->window) {
        bk_sys_free(st);
        return;
    }
