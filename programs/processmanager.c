...
static void pm_sample(processmanager_state_t *st) {
    ...
    if (now - st->last_sample < 250) return;
    ...
    st->cpu[PM_HISTORY - 1] = task_cpu_usage();
...
static void pm_graph(gui_surface_t *surface, gui_rect_t r,
    ...
    gui_gfx_fill_rect(surface, r, 0x00182020);
    gui_gfx_draw_rect(surface, r, 0x00607070);
...
static void pm_draw_button(gui_surface_t *surface, gui_rect_t rect,
    ...
    gui_gfx_fill_rect(surface, rect, border);
    gui_gfx_fill_rect(surface, inner, face);
    if (pressed) {
        gui_gfx_fill_rect(surface, (gui_rect_t){inner.x, inner.y, inner.w, 1},
                          0x00606060);
        gui_gfx_fill_rect(surface, (gui_rect_t){inner.x, inner.y, 1, inner.h},
                          0x00606060);
    } else {
        gui_gfx_fill_rect(surface, (gui_rect_t){inner.x, inner.y, inner.w, 1},
                          0x00FFFFFF);
    ...
    int text_x = rect.x + (rect.w - (int)gui_font_text_width(label)) / 2;
    if (text_x < rect.x + 3) text_x = rect.x + 3;
    gui_font_draw_string_clipped(surface, text_x, rect.y + 5, label,
...
static void pm_layout_build(const processmanager_state_t *st,
    ...
    int x = bounds.x + 8;
    int y = bounds.y + GUI_TITLEBAR_HEIGHT + 6;
    int w = bounds.w - 16;
...
static int pm_hit_row(const pm_layout_t *layout, uint32_t count,
    ...
    for (uint32_t row = 0; row < count && (int)row < layout->row_count; row++)
        if (gui_rect_contains(pm_row_rect(layout, (int)row), x, y))
            return (int)row;
    return -1;
}
...
static int pm_selected_index(const processmanager_state_t *st,
    ...
    for (uint32_t i = 0; i < count; i++) {
        const task_t *task = task_get(row_to_task[i]);
...
static uint32_t pm_collect_rows(uint32_t *row_to_task, uint32_t max) {
    uint32_t total = task_count();
    uint32_t count = 0;
    for (uint32_t i = 0; i < total && count < max; i++) {
        if (!task_get(i)) continue;
        row_to_task[count++] = i;
    }
    return count;
}
...
static void pm_content(gui_window_t *window UNUSED, gui_surface_t *surface,
    ...
    mm_get_system_info(&info);
    pm_number(number, task_cpu_usage());
    kstrcat(cpu_label, number);
    kstrcat(cpu_label, "%");
    pm_size_label(mem_text, (uint32_t)info.used_bytes);
    pm_size_label(total_text, (uint32_t)info.total_bytes);
    kstrcat(ram_label, mem_text);
    kstrcat(ram_label, " / ");
    kstrcat(ram_label, total_text);

    gui_font_draw_string_clipped(surface, layout.cpu_label.x, layout.cpu_label.y,
                                 cpu_label, 0x00102020, layout.cpu_label);
    pm_graph(surface, layout.cpu_graph, st->cpu, 0x0048E080);
    gui_font_draw_string_clipped(surface, layout.ram_label.x, layout.ram_label.y,
                                 ram_label, 0x00102020, layout.ram_label);
    pm_graph(surface, layout.ram_graph, st->ram, 0x0060A8F0);

    gui_gfx_fill_rect(surface, layout.header, 0x00889098);
    ...
    gui_font_draw_string_clipped(surface, layout.header.x + 4, layout.header.y + 4,
                                 "PID", 0x00FFFFFF,
                                 (gui_rect_t){layout.header.x + 2, layout.header.y + 2,
                                              pid_w, 12});
    gui_font_draw_string_clipped(surface, name_x, layout.header.y + 4,
                                 "PROCESO", 0x00FFFFFF,
                                 (gui_rect_t){name_x, layout.header.y + 2,
                                              state_x - name_x - 4, 12});
    gui_font_draw_string_clipped(surface, state_x, layout.header.y + 4,
                                 "ESTADO", 0x00FFFFFF,
                                 (gui_rect_t){state_x, layout.header.y + 2,
                                              state_w, 12});
    gui_font_draw_string_clipped(surface, mem_x, layout.header.y + 4,
                                 "RAM EST.", 0x00FFFFFF,
                                 (gui_rect_t){mem_x, layout.header.y + 2,
                                              mem_w, 12});

    for (uint32_t i = 0; i < count && (int)i < layout.row_count; i++) {
        const task_t *task = task_get(rows[i]);
        ...
        gui_gfx_fill_rect(surface, row_rect,
                          is_selected ? 0x00DCE9F7
                                      : ((i & 1U) ? 0x00E8E8E0 : 0x00F8F8F0));
        gui_font_draw_string_clipped(surface, row_rect.x + 4, row_rect.y + 4,
                                     number, 0x00203030,
                                     (gui_rect_t){row_rect.x + 2, row_rect.y + 2,
                                                  pid_w, 12});
        gui_font_draw_string_clipped(surface, name_x, row_rect.y + 4,
                                     pm_task_label(task), 0x00203030,
                                     (gui_rect_t){name_x, row_rect.y + 2,
                                                  state_x - name_x - 4, 12});
        gui_font_draw_string_clipped(surface, state_x, row_rect.y + 4,
                                     task_state_name(task->state),
                                     task->system ? 0x00505050 : 0x00007030,
                                     (gui_rect_t){state_x, row_rect.y + 2,
                                                  state_w, 12});
        gui_font_draw_string_clipped(surface, mem_x, row_rect.y + 4,
                                     mem_text, 0x00203030,
    ...
    if (selected >= 0) {
        const task_t *task = task_get(rows[selected]);
        pm_size_label(mem_text, task ? task->memory_bytes : 0);
        kstrcpy(footer, task ? pm_task_label(task) : "");
        kstrcat(footer, " | ");
        kstrcat(footer, task ? task_state_name(task->state) : "");
        kstrcat(footer, " | ");
        kstrcat(footer, mem_text);
    } else {
        kstrcpy(footer, "Tareas: ");
        pm_number(number, task_count());
        kstrcat(footer, number);
    }

    gui_font_draw_string_clipped(surface, layout.action.x + 2, layout.action.y + 5,
                                 footer, 0x00283C4A,
    ...
    pm_draw_button(surface, layout.button, "Finalizar",
                   selected >= 0 && pm_can_terminate(task_get(rows[selected])),
                   st->finish_pressed);
}
...
static bool pm_event(gui_window_t *window UNUSED, const gui_event_t *event,
    ...
    pm_validate_selection(st, rows, count);
    selected = pm_selected_index(st, rows, count);
    inside = gui_rect_contains(st->window->bounds, event->x, event->y);

    if (event->type == GUI_EVENT_MOUSE_DOWN) {
        if (inside && selected >= 0 &&
            pm_can_terminate(task_get(rows[selected])) &&
            gui_rect_contains(layout.button, event->x, event->y)) {
    ...
    if (st->finish_pressed) {
        bool pressed = gui_rect_contains(layout.button, event->x, event->y);
        st->finish_pressed = false;
        st->window->dirty = true;
        if (pressed && selected >= 0) {
            const task_t *task = task_get(rows[selected]);
            if (pm_can_terminate(task)) {
                task_request_exit(task->pid);
                return true;
            }
        }
    }
    ...
    if (row >= 0 && (uint32_t)row < count) {
        const task_t *task = task_get(rows[row]);
        if (task) {
...
static void pm_main(void *argument) {
    ...
    st->window = gui_desktop_create_window(st->desktop, 145, 35, 350, 290,
                                           "Administrador de procesos");
    if (st->window) {
        gui_window_set_min_size(st->window, 270, 220);
        gui_window_set_content(st->window, pm_content, st);
        gui_window_set_event_handler(st->window, pm_event, st);
...
void processmanager_open_from_desktop(gui_desktop_t *desktop) {
    ...
    st = (processmanager_state_t *)kzalloc(sizeof(*st));
    if (!st) return;
    st->desktop = desktop;
    g_processmanager = st;
    if (task_create("processmgr", pm_main, st) < 0) {
        pm_cleanup(st);
    }
}
