...
static void draw_button(bk_gui_surface_t *surface, bk_gui_rect_t rect,
    ...
    int group_width = (int)bk_gui_text_width(label);
    int text_x;
    ...
    if (icon && icon->pixels) {
        bk_gui_surface_draw_image(surface,
            (bk_gui_rect_t){text_x, rect.y + 4, 16, 16}, rect, icon);
        text_x += 20;
    }
...
static void scandisk_paint(bk_gui_window_t *window UNUSED,
    ...
    bk_gui_surface_draw_text(surface, x, y, "@HCD99842F",
                             0x00002060U, 0, false);
    y += 20;
    ...
    draw_button(surface, scan_button, "@H5276B5A7",
                state->phase == SCANDISK_READY ||
                state->phase == SCANDISK_FAILED,
                state->button_icon_loaded[0] ? &state->button_icons[0] : NULL);
    draw_button(surface, repair_button,
                state->confirm_repair ? "@H22380468" : "@H8BC58B0C",
                repair_enabled,
                state->button_icon_loaded[1] ? &state->button_icons[1] : NULL);
}
...
void bleskernos_program_main(bk_gui_desktop_t *desktop) {
    ...
    state->button_icon_loaded[0] =
        bk_graphics_icon_load("Scandskw1", &state->button_icons[0]);
    state->button_icon_loaded[1] =
        bk_graphics_icon_load("Qfecheck111", &state->button_icons[1]);
    if (!bk_device_volume_info(&state->volume)) {
        for (int i = 0; i < 2; i++)
            if (state->button_icon_loaded[i])
                bk_gui_image_free(&state->button_icons[i]);
        bk_sys_free(state);
        return;
    }
