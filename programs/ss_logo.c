...
static void draw_stars(gui_surface_t *surface, ss_logo_state_t *st) {
    ...
    for (uint32_t i = 0; i < 90; i++) {
        ...
        gui_gfx_putpixel(surface, x, y, shade);
    }
    st->seed = old_seed;
}
...
static void draw_logo(gui_surface_t *surface, int x, int y) {
    ...
    gui_gfx_fill_rect(surface, box, 0x00000000);
    gui_gfx_draw_rect(surface, box, 0x00FFFFFF);
    gui_gfx_draw_rect(surface, (gui_rect_t){x + 2, y + 2, 204, 54}, 0x000080FF);
    gui_gfx_fill_rect(surface, (gui_rect_t){x + 5, y + 5, 198, 48}, 0x00002040);
    gui_font_draw_string_scaled(surface, x + 18, y + 14,
                                "BlesKernOS", 0x00FFFFFF, 2);
    gui_font_draw_string(surface, x + 52, y + 39,
...
static void ss_logo_paint(gui_program_t *program, gui_desktop_t *desktop,
    ...
    gui_gfx_clear(surface, 0x00000000);
...
static void ss_logo_close(gui_desktop_t *desktop, ss_logo_state_t *st) {
    ...
    if (desktop && program) {
        st->program = NULL;
        gui_desktop_unregister_program(desktop, program);
    }
    kfree(st);
...
void ss_logo_open_from_desktop(gui_desktop_t *desktop) {
    ...
    st = (ss_logo_state_t *)kzalloc(sizeof(*st));
    if (!st) return;
    ...
    st->program = gui_desktop_register_program(desktop, "ss_logo", st,
                                               ss_logo_paint,
                                               ss_logo_event,
                                               NULL);
    if (!st->program) {
...
void ss_logo_main(gui_desktop_t *desktop) {
    ss_logo_open_from_desktop(desktop ? desktop : gui_get_desktop());
}
