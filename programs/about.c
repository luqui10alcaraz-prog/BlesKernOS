...
static bool about_load_logo(about_state_t *st) {
    if (!st) return false;
    return gui_gif_load_animation(&st->logo, "/ABOUNT.GIF");
}
...
static int about_centered_text_x(int area_x, int area_w, const char *text) {
    ...
    text_w = (int)gui_font_text_width(text);
...
static void about_content(gui_window_t *window UNUSED,
    ...
    if (logo && logo->pixels) {
        program_draw_icon_pixels(surface, logo_x, y, logo->pixels,
                                 logo->width, logo->height);
    } else if (st->icon) {
        program_draw_icon_pixels(surface, logo_x, y, st->icon, 28, 28);
    } else {
        gui_gfx_fill_rect(surface, (gui_rect_t){logo_x, y, 28, 28}, 0x00204070);
        gui_font_draw_string_scaled(surface, logo_x + 7, y + 6, "B",
                                    0x00FFFFFF, 2);
    }
    ...
    if (info_box.w > 0 && info_box.h > 0) {
        gui_gfx_fill_rect(surface, info_box, 0x00FFFFFF);
        gui_gfx_draw_rect(surface, info_box, 0x0090A0B0);
        ...
        info_y = info_box.y + 6;
        gui_font_draw_string(surface, info_box.x + 10, info_y,
                             "Kernel:", 0x00203040, 0, false);
        gui_font_draw_string_clipped(surface, info_box.x + 62, info_y,
                                     "BK1", 0x00203040, info_clip);
        gui_font_draw_string(surface, info_box.x + 10, info_y + 18,
                             "CPU:", 0x00203040, 0, false);
        gui_font_draw_string_clipped(surface, info_box.x + 62, info_y + 18,
                                     st->cpu, 0x00203040, info_clip);
        gui_font_draw_string(surface, info_box.x + 10, info_y + 32,
                             "RAM:", 0x00203040, 0, false);
        gui_font_draw_string_clipped(surface, info_box.x + 62, info_y + 32,
...
static void about_cleanup(about_state_t *st) {
    ...
    if (st->icon) kfree(st->icon);
    gui_gif_animation_free(&st->logo);
...
static void about_main(void *argument) {
    ...
    task_set_memory_hint(sizeof(*st) + (st->icon ? ABOUT_ICON_BYTES : 0) +
                         about_logo_bytes(st));
    st->window = gui_desktop_create_window(
        st->desktop, 125, 58, 390, st->logo.frame_count ? 250 : 165,
        "Sobre BlesKernOS 1.0");
    if (st->window) {
        gui_window_set_content(st->window, about_content, st);
        gui_window_set_min_size(st->window,
    ...
    if (st->logo.frame_count > 1)
        st->logo_next_tick = pit_get_ticks() + about_frame_delay_ticks(st, 0);

    while (!task_exit_requested()) {
        if (!st->window || !st->window->listed) break;
        if (about_advance_logo(st, pit_get_ticks()) && st->window)
            st->window->dirty = true;
        about_ram_text(st->ram);
        task_sleep(st->logo.frame_count > 1 ? 1U : 8U);
    }
...
void about_open(gui_desktop_t *desktop) {
    ...
    if (g_about) {
        if (g_about->window) {
            if (!g_about->window->visible)
                gui_window_restore(g_about->window);
            gui_desktop_raise_window(desktop, g_about->window);
            g_about->window->dirty = true;
        }
        return;
    }

    st = (about_state_t *)kzalloc(sizeof(*st));
    if (!st) return;
    st->desktop = desktop;
    ...
    g_about = st;
    if (task_create("about", about_main, st) < 0) {
        about_cleanup(st);
    }
}
