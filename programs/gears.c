...
static bool gears_init_gl(gears_state_t *st)
{
    ...
    st->gl = bk_tinygl_create(GEARS_W, GEARS_H);
    if (!st->gl) return false;
    st->gl_ready = true;
...
{
    if (!st || !st->gl_ready || !st->gl) return;
     ...
     */
    glTextSize(GL_TEXT_SIZE8x8);
    glDrawText((GLubyte *)"@H54E3103D", 4, 4, 0xFFFFFF);
    (void)bk_tinygl_present(st->gl);
    st->frames++;
}
    ...
    for (int y = 0; y < GEARS_H; y++) {
        ...
        for (int x = 0; x < GEARS_W; x++) {
            ...
            bk_gui_gfx_putpixel(surface, sx, sy,
                             bk_tinygl_pixel_rgb(bk_tinygl_pixels(st->gl)[y * GEARS_W + x]));
        }
    }
}
