...
typedef struct {
    ...
    char image_names[VFS_MAX_DIR_ENTRIES][13];
} viewer_state_t;
...
static bool viewer_load_bmp(gui_image_t *image, const char *path) {
    ...
    if (!vfs_read_all(path, &raw, &size) || !raw || size < 54) return false;
    data = (uint8_t *)raw;
    offset = rd32(data + 10);
    ...
    image->pixels = (uint32_t *)kmalloc((uint32_t)width * height * 4U);
    if (!image->pixels) {
        kfree(raw);
        return false;
    }
...
static bool ends_with(const char *s, const char *upper, const char *lower) {
    ...
    return dot && (kstrcmp(dot, upper) == 0 || kstrcmp(dot, lower) == 0);
}
...
static void viewer_split_path(const char *path, char *directory,
    ...
    if (directory) {
        ...
        if (dir_len == 0) {
            ...
            kmemcpy(directory, path, dir_len);
            directory[dir_len] = '\0';
        }
    }
    if (name) {
        kstrncpy(name, slash + 1, 12);
        name[12] = '\0';
    }
}
...
static void viewer_join_path(char *out, const char *directory,
    ...
    if (!out) return;
    kstrncpy(out, directory && *directory ? directory : "/", VFS_MAX_PATH - 1);
    ...
    kstrncpy(out + len, name ? name : "", VFS_MAX_PATH - len - 1);
}
...
static void viewer_update_title(viewer_state_t *st) {
    ...
    kstrncpy(st->window->title, "Visor: ", sizeof(st->window->title) - 1);
    st->window->title[sizeof(st->window->title) - 1] = '\0';
    used = kstrlen(st->window->title);
    kstrncpy(st->window->title + used, name,
...
static void viewer_refresh_gallery(viewer_state_t *st) {
    ...
    if (!vfs_listdir(st->directory, entries, VFS_MAX_DIR_ENTRIES, &count))
        return;
    ...
    for (uint32_t i = 0; i < count && st->image_count < VFS_MAX_DIR_ENTRIES;
        ...
        if (entries[i].type != VFS_NODE_FILE ||
            ...
            continue;
        kstrncpy(st->image_names[st->image_count], entries[i].name, 12);
        st->image_names[st->image_count][12] = '\0';
        if (kstrcmp(entries[i].name, current_name) == 0)
...
static gui_rect_t viewer_image_area(const viewer_state_t *st) {
    ...
    int y = st->window->bounds.y + GUI_TITLEBAR_HEIGHT + VIEWER_TOOLBAR_H;
    int w = st->window->bounds.w - VIEWER_MARGIN * 2;
    int h = st->window->bounds.h - GUI_TITLEBAR_HEIGHT -
            VIEWER_TOOLBAR_H - VIEWER_STATUS_H - VIEWER_MARGIN;
    if (w < 1) w = 1;
    if (h < 1) h = 1;
...
static void viewer_update_controls(viewer_state_t *st) {
    ...
    if (widget) {
        ...
        kstrncpy(widget->text, st->playing ? "Pausa" : "Play",
...
static void viewer_free_media(viewer_state_t *st) {
    if (!st) return;
    gui_image_free(&st->bmp);
    gui_gif_animation_free(&st->gif);
    st->animated = false;
    st->loaded = false;
    st->playing = false;
    st->frame = 0;
...
static void viewer_load_current(viewer_state_t *st) {
    ...
    if (ends_with(st->path, ".GIF", ".gif")) {
        st->loaded = gui_gif_load_animation_limited(&st->gif, st->path, 64);
    ...
    st->next_frame = pit_get_ticks() + 100;
...
static void viewer_open_index(viewer_state_t *st, int index) {
    ...
    kstrncpy(st->path, next_path, sizeof(st->path) - 1);
...
static void viewer_draw(gui_window_t *window UNUSED, gui_surface_t *surface,
    ...
    gui_gfx_fill_rect(surface,
        (gui_rect_t){st->window->bounds.x + 2,
                     ...
                     st->window->bounds.h - GUI_TITLEBAR_HEIGHT -
                         VIEWER_TOOLBAR_H - 3},
        0x00282828);
    gui_gfx_fill_rect(surface,
        (gui_rect_t){st->window->bounds.x + 2,
                     st->window->bounds.y + GUI_TITLEBAR_HEIGHT + VIEWER_TOOLBAR_H - 1,
                     st->window->bounds.w - 4, 1},
        0x00484848);
    gui_gfx_fill_rect(surface, status_rect, 0x001C1C1C);
    gui_gfx_draw_rect(surface, status_rect, 0x00484848);

    if (!st->loaded) {
        gui_font_draw_string(surface, area.x + 12, area.y + 12,
                             "No se pudo abrir la imagen",
                             0x00FFFFFF, 0, false);
        gui_font_draw_string_clipped(surface, area.x + 12, area.y + 30,
                                     st->path, 0x00B0B0B0,
    ...
    gui_gfx_draw_rect(surface, area, 0x00565656);

    {
        ...
        for (int y = clip_y0; y < clip_y1; y++) {
            ...
            for (int x = clip_x0; x < clip_x1; x++) {
                ...
                if ((color >> 24) != 0)
                    gui_gfx_putpixel(surface, x, y, color & 0x00FFFFFF);
            }
        }
    }

    viewer_build_status(st, status, sizeof(status));
    gui_font_draw_string_clipped(surface, status_rect.x + 6, status_rect.y + 5,
...
static bool viewer_event(gui_window_t *window UNUSED, const gui_event_t *event,
    ...
    if (event->type == GUI_EVENT_MOUSE_DOWN &&
        image && gui_rect_contains(area, event->x, event->y) &&
...
static void viewer_main(void *argument) {
    ...
    viewer_load_current(st);
    st->window = gui_desktop_create_window(st->desktop, 82, 36, 440, 320,
                                           "Visor de imagenes");
    if (st->window) {
        gui_widget_t *button;
        gui_window_set_content(st->window, viewer_draw, st);
        gui_window_set_event_handler(st->window, viewer_event, st);
        gui_window_set_min_size(st->window, 280, 190);
...
void imageviewer_open(gui_desktop_t *desktop, const char *path) {
    ...
    st = (viewer_state_t *)kzalloc(sizeof(*st));
    if (!st) return;
    st->desktop = desktop;
    st->zoom_pct = 100;
    ...
    st->path[sizeof(st->path) - 1] = '\0';
    g_viewer = st;
    if (task_create("imageviewer", viewer_main, st) < 0) viewer_cleanup(st);
}
