...
static void viewer_update_controls(viewer_state_t *st) {
    ...
    if (widget) {
        ...
        bk_runtime_strncpy(widget->text, st->playing ? "@H40F9B8E1" : "@H29AB6F83",
...
static void viewer_main(void *argument) {
    ...
    if (st->window) {
        ...
        if (button) {
            st->prev_id = button->id;
            (void)bk_gui_widget_set_icon(button, "ArrowLeft");
        }
        button = bk_gui_widget_create(st->desktop, st->window, GUI_WIDGET_BUTTON,
            (gui_rect_t){34, 6, 24, 18}, "", viewer_next_cb);
        if (button) {
            st->next_id = button->id;
            (void)bk_gui_widget_set_icon(button, "ArrowRight");
        }
        button = bk_gui_widget_create(st->desktop, st->window, GUI_WIDGET_BUTTON,
            (gui_rect_t){78, 6, 24, 18}, "-", viewer_zoom_out_cb);
        if (button) {
            st->zoom_out_id = button->id;
            (void)bk_gui_widget_set_icon(button, "FindDoc");
        }
        button = bk_gui_widget_create(st->desktop, st->window, GUI_WIDGET_BUTTON,
            (gui_rect_t){106, 6, 24, 18}, "+", viewer_zoom_in_cb);
        if (button) {
            st->zoom_in_id = button->id;
            (void)bk_gui_widget_set_icon(button, "FindDoc");
        }
        button = bk_gui_widget_create(st->desktop, st->window, GUI_WIDGET_BUTTON,
            (gui_rect_t){142, 6, 56, 18}, "@HA9EBA811", viewer_fit_cb);
        if (button) {
            st->fit_id = button->id;
            (void)bk_gui_widget_set_icon(button, "Fullscrn");
        }
        button = bk_gui_widget_create(st->desktop, st->window, GUI_WIDGET_BUTTON,
            (gui_rect_t){202, 6, 46, 18}, "100%", viewer_actual_cb);
        if (button) {
            st->actual_id = button->id;
            (void)bk_gui_widget_set_icon(button, "Camera");
        }
        button = bk_gui_widget_create(st->desktop, st->window, GUI_WIDGET_BUTTON,
            (gui_rect_t){252, 6, 54, 18}, "@H40F9B8E1", viewer_play_cb);
        if (button) {
            st->play_id = button->id;
            (void)bk_gui_widget_set_icon(button, "Mplayer10");
        }
        viewer_update_title(st);
        viewer_update_controls(st);
    }
