...
static bool ss_pipes_init_gl(ss_pipes_state_t *st) {
    ...
    st->pixels = (PIXEL *)kzalloc(pixel_count * sizeof(PIXEL));
...
void ss_pipes_open_from_desktop(gui_desktop_t *desktop) {
    ...
    st = (ss_pipes_state_t *)kzalloc(sizeof(*st));
    if (!st) return;

    st->desktop = desktop;
    st->active = true;
    st->seed = pit_get_ticks() ^ 0x51F1E50U;
    st->last_tick = pit_get_ticks();
    st->last_grow_tick = st->last_tick;
    ss_pipes_configure_resolution(st, desktop);

    st->program = gui_desktop_register_program(desktop, "ss_pipes", st,
                                               ss_pipes_paint,
                                               ss_pipes_event,
                                               NULL);
    if (!st->program) {
...
void ss_pipes_main(gui_desktop_t *desktop) {
    ss_pipes_open_from_desktop(desktop ? desktop : gui_get_desktop());
}
