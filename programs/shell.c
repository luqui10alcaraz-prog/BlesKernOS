...
static int cmd_help(int argc UNUSED, char **argv UNUSED) {
    ...
    kprintf("  version uptime ticks beep panic cpu disks fatinfo mount audio\n");
    kprintf("  cdinfo cdread usb\n");
    kprintf("  pwd cd ls cat open read close mkdir pci video gfx vesa mouse\n");
    kprintf("  lsmem lsirq gdt idt heap tasks\n");
    return 0;
}
...
static int cmd_panic(int argc, char **argv) {
    panic_show(argc > 1 ? argv[1] : "PANIC SOLICITADO",
               3, 0, (uint32_t)cmd_panic);
}
...
static int cmd_disks(int argc UNUSED, char **argv UNUSED) {
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        if (!dev) continue;
        kprintf("%s\tbloques=%u\tsector=%u\ttipo=%s\t%s\n",
                dev->name,
                dev->sector_count,
                dev->sector_size,
...
static block_device_t *find_block_device(const char *name, block_device_type_t type) {
    block_device_t *dev;

    if (!name) return NULL;
    dev = block_get(name);
    if (!dev || dev->type != type) return NULL;
    return dev;
}

static void print_hex_byte(uint8_t value) {
    static const char *hex = "0123456789ABCDEF";
    vga_putchar(hex[(value >> 4) & 0x0F]);
    vga_putchar(hex[value & 0x0F]);
}

static void shell_hexdump(const uint8_t *buffer, uint32_t length) {
    for (uint32_t off = 0; off < length; off += 16U) {
        vga_puthex(off);
        vga_puts(": ");
        for (uint32_t i = 0; i < 16U; i++) {
            if (off + i < length) print_hex_byte(buffer[off + i]);
            else vga_puts("  ");
            vga_putchar(' ');
        }
        vga_putchar(' ');
        for (uint32_t i = 0; i < 16U && off + i < length; i++) {
            char c = (char)buffer[off + i];
            if (c < 32 || c > 126) c = '.';
            vga_putchar(c);
        }
        vga_putchar('\n');
...
static int cmd_beep(int argc, char **argv) {
    ...
    if (argc > 3) {
        kprintf("Uso: beep [frecuencia_hz] [duracion_ms]\n");
        kprintf("     beep stop\n");
        return 1;
    }
    ...
    if (argc > 1 && kstrcmp(argv[1], "stop") == 0) {
        sound_stop();
        kprintf("Sonido detenido.\n");
        return 0;
    }

    if (argc > 1) hz = parse_uint(argv[1]);
    if (argc > 2) duration_ms = parse_uint(argv[2]);
    if (hz <= 0 || duration_ms < 0) {
        kprintf("Uso: beep [frecuencia_hz] [duracion_ms]\n");
        kprintf("     beep stop\n");
        return 1;
    }
    ...
    if (duration_ms == 0) {
        sound_play((uint32_t)hz);
        kprintf("Tono activo: %u Hz\n", hz);
        return 0;
    }

    sound_beep((uint32_t)hz, (uint32_t)duration_ms);
    kprintf("Beep: %u Hz por %u ms\n", hz, duration_ms);
    return 0;
}
...
static int cmd_audio(int argc, char **argv) {
    if (argc == 1 || kstrcmp(argv[1], "info") == 0) {
        kprintf("Audio PCM: %s\n", sound_pcm_name());
        kprintf("  sb16=%s busy=%s\n",
                sound_has_sb16() ? "yes" : "no",
                sound_sb16_is_busy() ? "yes" : "no");
        if (!sound_has_sb16()) {
            kprintf("  Usa beep para PC speaker o ejecuta QEMU con SB16.\n");
        }
        return 0;
    }

    if (kstrcmp(argv[1], "tone") == 0) {
        int hz;
        int duration_ms = 500;

        if (argc < 3) {
            kprintf("Uso: audio tone <frecuencia_hz> [duracion_ms]\n");
            return 1;
        }
        hz = parse_uint(argv[2]);
        if (argc > 3) duration_ms = parse_uint(argv[3]);
        if (hz <= 0 || duration_ms <= 0) {
            kprintf("Parametros invalidos.\n");
            return 1;
        }
        if (!sound_sb16_play_tone((uint32_t)hz, (uint32_t)duration_ms)) {
            kprintf("No se pudo reproducir por SB16.\n");
            return 1;
        }
        kprintf("SB16: %u Hz por %u ms\n", hz, duration_ms);
        return 0;
    }

    if (kstrcmp(argv[1], "stop") == 0) {
        sound_stop();
        kprintf("Audio detenido.\n");
        return 0;
    }

    kprintf("Uso: audio [info|tone <hz> [ms]|stop]\n");
    return 1;
}

static int cmd_cdinfo(int argc, char **argv) {
    ...
    for (uint32_t i = 0; i < block_count(); i++) {
        block_device_t *dev = block_at(i);
        uint32_t total_mb;
        if (!dev || dev->type != BLOCK_DEVICE_ATAPI) continue;
        if (requested && kstrcmp(requested, dev->name) != 0) continue;

        (void)ata_refresh_media(dev->name);
        found = true;
        if (dev->sector_count == 0) {
            kprintf("%s\tatapi\tsector=%u\tmedio=no-listo\n",
                    dev->name, dev->sector_size);
            continue;
        }

        total_mb = (dev->sector_count * dev->sector_size) / (1024U * 1024U);
        kprintf("%s\tatapi\tbloques=%u\tsector=%u\t~%u MB\n",
                dev->name, dev->sector_count, dev->sector_size, total_mb);
    }

    if (!found) {
        kprintf(requested ? "No se encontro la unidad ATAPI solicitada.\n"
                          : "No hay unidades ATAPI detectadas.\n");
        return 1;
    }
    return 0;
}

static int cmd_cdread(int argc, char **argv) {
    block_device_t *dev;
    uint32_t lba;
    uint32_t bytes_to_dump = 128;
    uint8_t *buffer;

    if (argc < 3) {
        kprintf("Uso: cdread <cd0> <lba> [bytes]\n");
        return 1;
    }

    dev = find_block_device(argv[1], BLOCK_DEVICE_ATAPI);
    lba = (uint32_t)parse_number(argv[2]);
    if (!dev || (int)lba < 0) {
        kprintf("Unidad o LBA invalidos.\n");
        return 1;
    }
    if (argc > 3) bytes_to_dump = (uint32_t)parse_number(argv[3]);
    if (bytes_to_dump == 0 || bytes_to_dump > dev->sector_size) {
        bytes_to_dump = dev->sector_size;
    }

    (void)ata_refresh_media(dev->name);
    buffer = (uint8_t *)kmalloc(dev->sector_size);
    if (!buffer) {
        kprintf("Sin memoria para leer el sector.\n");
        return 1;
    }
    if (!block_read(dev, lba, 1, buffer)) {
        kfree(buffer);
        kprintf("Fallo leyendo %s LBA %u.\n", dev->name, lba);
        return 1;
    }

    kprintf("%s LBA=%u bytes=%u\n", dev->name, lba, dev->sector_size);
    shell_hexdump(buffer, bytes_to_dump);
    kfree(buffer);
    return 0;
}
...
static int cmd_usb(int argc, char **argv) {
    ...
    if (argc > 1 && kstrcmp(argv[1], "list") != 0) {
        kprintf("Uso: usb [list]\n");
        return 1;
    }

    for (uint32_t i = 0; i < pci_device_count(); i++) {
        const pci_device_t *dev = pci_device_at(i);
        if (!dev || dev->class_code != 0x0C || dev->subclass != 0x03) continue;
        found = true;
        kprintf("%u: %u:%u.%u %s irq=%u pin=%u\n",
                i,
                dev->bus,
                dev->slot,
                dev->function,
    ...
    if (!found) {
        kprintf("No hay controladores USB PCI detectados.\n");
        return 1;
    }

    kprintf("Nota: stack de almacenamiento USB aun no implementado.\n");
    return 0;
}
...
static int cmd_lsmem(int argc UNUSED, char **argv UNUSED) {
    heap_info_t info;
    mm_get_info(&info);
    kprintf("Mapa de memoria administrada:\n");
    kprintf("  heap start=0x%x end=0x%x size=%u\n",
            HEAP_START, HEAP_END, HEAP_SIZE);
    kprintf("  usados=%u libres=%u total=%u\n",
            info.used_bytes, info.free_bytes, info.total_bytes);
    return 0;
}

static int cmd_lsirq(int argc UNUSED, char **argv UNUSED) {
    uint8_t master = inb(PIC1_DATA);
    uint8_t slave = inb(PIC2_DATA);
    kprintf("IRQ PIC master mask=0x%x slave mask=0x%x\n", master, slave);
    for (uint32_t irq = 0; irq < 16; irq++) {
        uint8_t mask = irq < 8 ? master : slave;
        uint8_t bit = (uint8_t)(1 << (irq & 7));
        kprintf("  IRQ%u: %s\n", irq, (mask & bit) ? "masked" : "enabled");
    }
    return 0;
}

static int cmd_gdt(int argc UNUSED, char **argv UNUSED) {
    struct {
        uint16_t limit;
        uint32_t base;
    } PACKED gdtr;
    __asm__ volatile ("sgdt %0" : "=m"(gdtr));
    kprintf("GDT base=0x%x limit=%u entries=%u\n",
            gdtr.base, gdtr.limit, (gdtr.limit + 1) / 8);
    return 0;
}

static int cmd_idt(int argc UNUSED, char **argv UNUSED) {
    idt_ptr_t idtr;
    __asm__ volatile ("sidt %0" : "=m"(idtr));
    kprintf("IDT base=0x%x limit=%u entries=%u\n",
            idtr.base, idtr.limit, (idtr.limit + 1) / sizeof(idt_entry_t));
    return 0;
}

static int cmd_heap(int argc UNUSED, char **argv UNUSED) {
    heap_info_t info;
    mm_get_info(&info);
    kprintf("Heap blocks=%u used_blocks=%u free_blocks=%u\n",
            info.total_blocks, info.used_blocks, info.free_blocks);
    kprintf("Heap bytes used=%u free=%u\n", info.used_bytes, info.free_bytes);
    return 0;
}

static int cmd_tasks(int argc UNUSED, char **argv UNUSED) {
    kprintf("PID Estado       Memoria Nombre\n");
    for (uint32_t i = 0; i < task_count(); i++) {
        const task_t *task = task_get(i);
        if (!task) continue;
        kprintf("%u  %s", task->pid, task_state_name(task->state));
        int pad = 11 - (int)kstrlen(task_state_name(task->state));
        while (pad-- > 0) vga_putchar(' ');
        kprintf("%u KB   %s\n", task->memory_bytes / 1024U, task->name);
    }
    return 0;
}
...
static const shell_cmd_t commands[] = {
    ...
    {"beep", "Reproduce un tono por PC speaker", cmd_beep},
    {"audio", "Prueba audio PCM por SB16", cmd_audio},
    ...
    {"cdinfo", "Muestra capacidad de unidades ATAPI", cmd_cdinfo},
    {"cdread", "Lee un sector de una unidad ATAPI", cmd_cdread},
    ...
    {"usb", "Lista controladores USB PCI detectados", cmd_usb},
    {"lsmem", "Muestra el mapa del heap", cmd_lsmem},
    {"lsirq", "Muestra mascaras y lineas IRQ", cmd_lsirq},
    {"gdt", "Muestra el registro GDT", cmd_gdt},
    {"idt", "Muestra el registro IDT", cmd_idt},
    {"heap", "Muestra uso y bloques del heap", cmd_heap},
    {"tasks", "Muestra las tareas del sistema", cmd_tasks},
    {NULL, NULL, NULL}
};
...
void shell_execute_line(const char *line) {
    ...
    if (!line) return;
    kstrncpy(command, line, sizeof(command) - 1);
