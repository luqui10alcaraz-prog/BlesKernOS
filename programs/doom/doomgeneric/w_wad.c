...
static wad_file_t *primary_wad_file;
...
static void ExtendLumpInfo(int newnumlumps)
{
    ...
    newlumpcache = calloc(newnumlumps, sizeof(void *));

    if (newlumpinfo == NULL || newlumpcache == NULL)
    {
    ...
    for (i = 0; i < numlumps && i < newnumlumps; ++i)
    {
        ...
        if (lumpcache != NULL && lumpcache[i] != NULL)
        {
            newlumpcache[i] = lumpcache[i];
            Z_ChangeUser(newlumpcache[i], &newlumpcache[i]);
        }
...
wad_file_t *W_AddFile (char *filename)
{
    ...
    if (primary_wad_file == NULL)
    {
        primary_wad_file = wad_file;
    }
...
int W_CheckNumForName (char* name)
{
    ...
    if (lumphash != NULL)
    {
        ...
        {
            if (lump_p < lumpinfo || lump_p >= lumpinfo + numlumps)
            {
                printf("[WAD] hash corrupto para %.8s; usando busqueda lineal\n",
                       name);
                hash_valid = false;
                break;
            }
        ...
        /*
         * A chain can be structurally in range but still be incomplete if
         * its links were overwritten.  A hash miss is therefore not final:
         * verify it with the authoritative linear directory scan below.
         */
        (void) hash_valid;
    }

    // No hash table, or a damaged hash chain: use the safe linear path.
    {
...
void W_ReadLump(unsigned int lump, void *dest)
{
    ...
    if (primary_wad_file != NULL && l->wad_file != primary_wad_file)
    {
        l->wad_file = primary_wad_file;
    }
    I_BeginRead ();
...
void *W_CacheLumpNum(int lumpnum, int tag)
{
    ...
    if (primary_wad_file != NULL && primary_wad_file->mapped != NULL)
    {
        // Memory mapped file, return from the mmapped region.

        result = primary_wad_file->mapped + lump->position;
    }
    else if (lumpcache[lumpnum] != NULL)
    {
        // Already cached, so just switch the zone tag.

        result = lumpcache[lumpnum];
        Z_ChangeTag(result, tag);
    }
    else
    {
        ...
        lumpcache[lumpnum] =
            Z_Malloc(W_LumpLength(lumpnum), tag, &lumpcache[lumpnum]);
