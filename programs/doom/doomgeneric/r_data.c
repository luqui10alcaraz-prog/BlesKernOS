...
void R_InitTextures (void)
{
    ...
    name[8] = 0;
    i = W_GetNumForName(DEH_String("PNAMES"));
    names = W_CacheLumpNum(i, PU_STATIC);
	...
	for (j=0 ; j<texture->patchcount ; j++, mpatch++, patch++)
	{
            ...
            {
                I_Error("R_InitTextures: patch %i fuera de PNAMES (%i)",
                        patch_index, nummappatches);
            }

	    patch->originx = SHORT(mpatch->originx);
	    patch->originy = SHORT(mpatch->originy);
            M_StringCopy(name, name_p + patch_index * 8, sizeof(name));
	    patch->patch = W_CheckNumForName(name);
	    if (patch->patch == -1)
	    {
		I_Error ("R_InitTextures: Missing patch '%s' (PNAMES %i) "
                         "in texture %.8s",
			 name, patch_index, texture->name);
	    }
	}		
