/* Restored generated LibCSS property implementation. */
		...
		css_select_state *state)
{
	...
	if (hasFlagValue(opv) == false) {
		switch (getValue(opv)) {
		case POSITION_STATIC: value = CSS_POSITION_STATIC; break;
		case POSITION_RELATIVE: value = CSS_POSITION_RELATIVE; break;
		case POSITION_ABSOLUTE: value = CSS_POSITION_ABSOLUTE; break;
		case POSITION_FIXED: value = CSS_POSITION_FIXED; break;
		}
	}
...
css_error css__copy_position(const css_computed_style *from,
		...
		const css_computed_style *child, css_computed_style *result)
{
	...
	return css__copy_position(type == CSS_POSITION_INHERIT ? parent : child,
			result);
}
