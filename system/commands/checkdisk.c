...
static int run(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        if (command_is(argv[i], "/fix") || command_is(argv[i], "-f")) {
            if (fix) return command_error("checkdisk", "@H3BB88A79");
            fix = true;
        } else if (!path_set) {
            path = argv[i];
            path_set = true;
    ...
    if (!bk_device_volume_info(&volume))
        return command_error("checkdisk", "@H92A33123");
    ...
    kprintf("Copia de arranque:       %s\n",
            structure.backup_boot_matches ? "coincide" : "@HA7DE1D94");
    ...
    if (fix) {
        ...
        kprintf("Copia de arranque:       %s\n",
                repair.backup_boot_repaired ? "correcta" : "@H52D5DBBE");
