...
static const char *devmgr_kind_text(devmgr_kind_t kind) {
    switch (kind) {
        ...
        case DEVMGR_KIND_DISPLAY: return "@H4C25F8DE";
        case DEVMGR_KIND_AUDIO: return "@H1A175EBB";
        case DEVMGR_KIND_INPUT: return "Entrada";
        case DEVMGR_KIND_RTC: return "Reloj";
        case DEVMGR_KIND_MEMORY: return "Memoria";
        case DEVMGR_KIND_KERNEL: return "@HAF21B997";
...
static const char *devmgr_section_text(devmgr_section_t section) {
    switch (section) {
        ...
        case DEVMGR_SECTION_SYSTEM: return "@H997580E2";
        case DEVMGR_SECTION_MEMORY: return "Memoria";
        case DEVMGR_SECTION_BLOCK: return "@H9C21998F";
        case DEVMGR_SECTION_DISPLAY: return "@HBB5B8D9D";
        case DEVMGR_SECTION_AUDIO: return "@HDFB53599";
        case DEVMGR_SECTION_INPUT: return "@H8DBC75E5";
        case DEVMGR_SECTION_RTC: return "@H49AA501C";
        case DEVMGR_SECTION_PCI: return "@H9202250F";
        default: return "@H619D37EF";
    }
}
...
static bool devmgr_loaded_driver(const char *name) {
    bk_driver_info_t driver;
    if (!name) return false;
    for (uint32_t i = 0; i < bk_device_driver_count(); i++) {
        if (bk_device_driver_info(i, &driver) &&
            bk_runtime_strcmp(driver.name, name) == 0) return true;
    }
    return false;
}
...
static const char *devmgr_driver_filename(const char *name,
    ...
    for (uint32_t i = 0; i < bk_device_driver_count(); i++) {
        ...
        char *destination;
        if (!bk_device_driver_info(i, &driver) ||
            bk_runtime_strcmp(driver.name, name) != 0 || !driver.path[0])
            continue;
        filename = driver.path;
        for (const char *cursor = driver.path; *cursor; cursor++) {
            if (*cursor == '/' || *cursor == '\\') filename = cursor + 1;
        }
        destination = result[result_index++ & 3U];
        devmgr_copy(destination, sizeof(result[0]), filename);
        return destination[0] ? destination : fallback;
    }
    return fallback;
}
...
static const devmgr_pci_id_t *devmgr_pci_lookup(const bk_pci_info_t *dev) {
    if (!dev) return NULL;
    for (uint32_t i = 0;
...
static void devmgr_pci_identity(const bk_pci_info_t *dev,
    ...
    snprintf(name, name_len, "@H748EC6A9",
...
static void devmgr_refresh(devmgr_state_t *st) {
    ...
    snprintf(detail, sizeof(detail), "@H455B28A5",
             bk_sys_api_version(), bk_proc_count());
    (void)devmgr_add(st, DEVMGR_KIND_KERNEL, 0, "@H132C24E0",
                     "kernel", "OK", detail);

    if (bk_sys_memory_info(&mem)) {
        snprintf(detail, sizeof(detail), "@HFF4D1298",
                 (uint32_t)(mem.total_bytes / 1024U),
                 (uint32_t)(mem.free_bytes / 1024U));
        (void)devmgr_add(st, DEVMGR_KIND_MEMORY, 0, "@HB7EAD0EE",
                         "memory", "OK", detail);
    }
    ...
    for (uint32_t i = 0; i < block_devices; i++) {
        ...
        block_driver = dev.type_name;
        if (dev.type == BK_BLOCK_ATAPI &&
            devmgr_loaded_driver("iso9660"))
            block_driver = "@H72BF6F64";
        snprintf(detail, sizeof(detail), "@HA84AE2D7",
                 dev.type_name, dev.sector_count,
                 dev.sector_size, dev.read_only ? "@H95E5221C" : "");
    ...
    for (uint32_t i = 0; i < pci_devices; i++) {
        ...
        if (dev.vendor_id == 0x1AF4 &&
            (dev.device_id == 0x1050 || dev.device_id == 0x1010) &&
            gfx_ok && gfx.mode == GFX_MODE_VIRTIO_GPU) {
            devmgr_copy(driver, sizeof(driver),
                        devmgr_driver_filename("virtio_gpu", "VIRTIOGPU.DVR"));
            status = "OK";
        } else if (dev.vendor_id == 0x15AD && dev.device_id == 0x0405 &&
            gfx_ok && gfx.mode == GFX_MODE_VMWARE_SVGA) {
            devmgr_copy(driver, sizeof(driver), devmgr_driver_filename("vmware_svga", "VMWARESVGA.DVR"));
            status = "OK";
        } else if (dev.vendor_id == 0x1002 && dev.device_id == 0x4C46 &&
                   gfx_ok && gfx.mode == GFX_MODE_ATI_RAGE128) {
            devmgr_copy(driver, sizeof(driver),
                        devmgr_driver_filename("ati_rage128_mobility",
                                               "ATIR128.DVR"));
            status = "OK";
        } else if (dev.vendor_id == 0x8086 && dev.class_code == 0x03 &&
                   gfx_ok && gfx.mode == GFX_MODE_INTEL_GMA9XX) {
            devmgr_copy(driver, sizeof(driver),
                        devmgr_driver_filename("intel_gma9xx", "INTELGMA.DVR"));
            status = "OK";
        } else if (dev.class_code == 0x03 && gfx_ok &&
                   gfx.mode == GFX_MODE_VESA_LFB &&
                   devmgr_loaded_driver("vesa")) {
            devmgr_copy(driver, sizeof(driver),
                        devmgr_driver_filename("vesa", "VESA.DVR"));
            status = "OK";
        }
        if (bk_runtime_strcmp(driver, "rtl8139") == 0 &&
            devmgr_loaded_driver("rtl8139")) {
            devmgr_copy(driver, sizeof(driver),
                        devmgr_driver_filename("rtl8139", "RTL8139.DVR"));
            status = "OK";
        }
        if (bk_runtime_strcmp(driver, "3c90x") == 0 &&
            devmgr_loaded_driver("3c90x")) {
            devmgr_copy(driver, sizeof(driver),
                        devmgr_driver_filename("3c90x", "3C90X.DVR"));
            status = "OK";
        }
    ...
    if (gfx_ok) {
        ...
        (void)devmgr_add(st, DEVMGR_KIND_DISPLAY, 0,
                         "@H48894648",
                         gfx.mode == GFX_MODE_VIRTIO_GPU
                            ? devmgr_driver_filename("virtio_gpu", "VIRTIOGPU.DVR")
                            : (gfx.mode == GFX_MODE_VMWARE_SVGA
                                ? devmgr_driver_filename("vmware_svga", "VMWARESVGA.DVR")
                                : (gfx.mode == GFX_MODE_ATI_RAGE128
                                    ? devmgr_driver_filename("ati_rage128_mobility",
                                                            "ATIR128.DVR")
                                    : (gfx.mode == GFX_MODE_INTEL_GMA9XX
                                        ? devmgr_driver_filename("intel_gma9xx",
                                                                "INTELGMA.DVR")
    ...
    snprintf(detail, sizeof(detail), "%s",
             bk_sound_pcm_available()
                ? (bk_sound_pcm_name() ? bk_sound_pcm_name() : "@H42B1D0B4")
                : "@H42449C8F");
    (void)devmgr_add(st, DEVMGR_KIND_AUDIO, 0, "@H4691F881",
                     bk_sound_pcm_name() &&
    ...
    if (bk_time_datetime(&now)) {
        ...
        (void)devmgr_add(st, DEVMGR_KIND_RTC, 0, "@H9541D93E",
...
static void devmgr_detail_paint(gui_window_t *window UNUSED,
    ...
    if (d->active_tab == 0) {
        ...
        devmgr_detail_line(s, x + 28, y + 1, "@H5830640E", d->item.name, clip);
        y += 24;
        devmgr_detail_line(s, x, y, "@H941F0DBF",
    ...
    if (d->item.kind == DEVMGR_KIND_BLOCK) {
        bk_block_info_t dev;
        if (bk_device_block_info(d->item.index, &dev)) {
            ...
            devmgr_detail_line(s, x + 14, y, "@H57D3A839", line, clip);
            y += 18;
            devmgr_detail_line(s, x + 14, y, "Tipo:", dev.type_name, clip);
            y += 18;
            devmgr_detail_line(s, x + 14, y, "Modo:",
                               dev.read_only ? "@H880428C8" : "Lectura/escritura",
                               clip);
        }
    } else if (d->item.kind == DEVMGR_KIND_PCI) {
        bk_pci_info_t dev;
        if (bk_device_pci_info(d->item.index, &dev)) {
            snprintf(line, sizeof(line), "%04x:%04x",
                     dev.vendor_id, dev.device_id);
            devmgr_detail_line(s, x + 14, y, "@H9607B30E", line, clip);
            y += 18;
            snprintf(line, sizeof(line), "@H51A018B9",
                     dev.bus, dev.slot, dev.function);
        ...
        snprintf(line, sizeof(line), "@H3041B300",
        ...
        snprintf(line, sizeof(line), "@H84B357B3",
        ...
        devmgr_detail_line(s, x + 14, y, "Recursos:", "@H47566538",
                           clip);
    }
}
...
static void devmgr_open_detail(devmgr_state_t *st, int index) {
    ...
    snprintf(title, sizeof(title), "@HE08EAF01", detail->item.name);
    ...
    snprintf(st->status, sizeof(st->status), "@H8CE576F6",
             detail->item.name);
    if (st->window) st->window->dirty = true;
...
static void devmgr_open_context(devmgr_state_t *st, int hit, int x, int y) {
    ...
    (void)bk_gui_window_context_add_item(st->window,
        DEVMGR_CONTEXT_PROPERTIES, "@H1933FA0F", device,
        devmgr_context_callback, st);
    (void)bk_gui_window_context_add_item(st->window,
        DEVMGR_CONTEXT_REFRESH, "@H70B6CA21", true,
        devmgr_context_callback, st);
    ...
    if (event->type == GUI_EVENT_MOUSE_DOWN) {
        ...
        if (hit >= 0) {
            ...
            if (st->last_clicked == hit &&
                st->last_click_tick &&
                now - st->last_click_tick <= DEVMGR_DBLCLICK_MS) {
