...
static void keyboard_layout(gui_window_t *window, uint32_t id) {
    ...
    cpl_write_text("/KEYBD.INI", id == st->layout_ids[0]
        ? "layout=ES\r\n" : "layout=US\r\n");
    snprintf(st->status, sizeof(st->status), "@H6119A79F", st->layout);
    window->dirty = true;
}
...
static void keyboard_paint(gui_window_t *window UNUSED, gui_surface_t *s,
    ...
    char line[80];
    cpl_draw_group(s, (gui_rect_t){x, y, 390, 128}, "@HBAEBF78B");
    bk_gui_font_draw_string(s, x + 14, y + 22, "@H7D353C7C", CPL_TEXT, 0, false);
...
void bleskernos_program_main(gui_desktop_t *desktop) {
    ...
    st->window = bk_gui_create_window(desktop, 102, 62, 422, 310, "@HF56C750D");
    if (!st->window) { bk_sys_free(st); return; }
    (void)bk_about_attach(st->window, desktop, &(bk_about_info_t){
        "@HF56C750D", "@H1C1E4EC2", "@H42EECB09",
        "@H7A28E1E5", "/ICONS/KEYBOARD.BMP"});
